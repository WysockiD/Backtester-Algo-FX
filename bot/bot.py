...
class Bot:
    ...
    def process_candles(self, triggered):
        if len(triggered) > 0:
            ...
            for p in triggered:
                print(f"process_candles triggered:{p}")
                last_time = self.candle_manager.timings[p].last_time
                print(f"process_candles last_time:{last_time}")
                trade_decision = get_trade_decision(last_time, p, Bot.GRANULARITY, self.api, 
                                                       self.trade_settings[p],  self.log_message)
                print(f"process_candles trade_decision:{trade_decision}")
                if trade_decision is not None and trade_decision.signal != defs.NONE:
                    print(f"process_candles place_trade:{trade_decision}")
                    print(f"process_candles place_trade:{trade_decision.pair} {trade_decision.signal} {trade_decision.gain} {trade_decision.loss} {trade_decision.sl} {trade_decision.tp} {self.api} {self.log_message} {self.log_to_error} {self.trade_risk}")
