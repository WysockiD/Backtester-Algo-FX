...
def get_trade_decision(candle_time, pair, granularity, api: OandaApi, 
    ...
    print(f"get_trade_decision: {pair} {candle_time} {granularity} {trade_settings}")
    ...
    print(f"get_trade_decision: df:{df}")

    if df is not None:
        last_row = process_candles(df, pair, trade_settings, log_message)
        print(f"get_trade_decision: last_row:{last_row}")
        print(TradeDecision(last_row))
